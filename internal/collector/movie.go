...
`, i+1, movie.Title, movie.VoteAverage, movie.ReleaseDate, truncate(movie.Overview, 150)))
	...
	for _, movie := range movies {
		...
		tags = append(tags, movie.Title+"평점")
	}

	// 타입별 추가 태그
	switch postType {
	case "now_playing":
		tags = append(tags, "현재상영영화", "상영중영화", "극장영화", "CGV", "메가박스", "롯데시네마")
	case "upcoming":
		tags = append(tags, "개봉예정영화", "신작영화", "개봉작", now.Format("01월")+"개봉영화")
	case "tv":
		tags = append(tags, "드라마", "드라마추천", "넷플릭스", "왓챠", "티빙", "인기드라마")
	}
